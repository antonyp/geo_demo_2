...
var mapOptions = {
    center: new google.maps.LatLng(-43.5131367,172.5990772),
    zoom: 9,
    //styles: snazzy['subtlegrayscale'],
    //styles: snazzy['shadesofgrey'],
    mapTypeControlOptions: {
      //mapTypeIds: ['cera', google.maps.MapTypeId.SATELLITE, google.maps.MapTypeId.ROADMAP]
    }
};
...
var ceraTypeOptions = {
  getTileUrl: function(coord, zoom) {
      ...
      return 'http://maps.cera.govt.nz/arcgis/rest/services/CERA/Christchurch_Aerial_20130716/MapServer/tile' +
          '/' + zoom +
          '/' + normalizedCoord.y +
          '/' + normalizedCoord.x;
  },
...
//var ceraMapType = new google.maps.ImageMapType(ceraTypeOptions);
...
var chchPostQuakeLayerId = '14243126420781440025-16850874272514742756';
var chchTopoLayerId = '14243126420781440025-04718830180594759816';
...
function initialise() {
    ...
    //map.mapTypes.set('cera', ceraMapType);
    //map.setMapTypeId('cera');

    //var ceraMapType = new gmaps.ags.MapType('http://maps.cera.govt.nz/arcgis/rest/services/CERA/Christchurch_Aerial_20130716/MapServer', { name: 'CERA Imagery' });
    //map.mapTypes.set('cera', ceraMapType);
    //map.setMapTypeId('cera');
    ...
    // Market Penetration
    layers['layer-chch-postquake'] = new google.maps.visualization.DynamicMapsEngineLayer({
                layerId: chchPostQuakeLayerId,
                map: null,
                suppressInfoWindows: true, /* we create our own, rather than using those from Maps Engine */
                clickable: true
            });
    $('#layer-chch-postquake').change(layerToggle);
    layers['layer-chch-topo'] = new google.maps.visualization.DynamicMapsEngineLayer({
                layerId: chchTopoLayerId,
                map: null,
                suppressInfoWindows: true, /* we create our own, rather than using those from Maps Engine */
                clickable: true
            });
    $('#layer-chch-topo').change(layerToggle);
...
function addSecondaryStores() {
    ...
    var primaryStoreInfoWindow;
    function secondaryStoresCallback(results, status) {
        if (status == google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
                var place = results[i];
                var marker = addPlace('layer-secondary-stores', place);

                if (marker != null) {
                    google.maps.event.addListener(marker, 'click', function() {
                        if (primaryStoreInfoWindow != undefined) {
                            primaryStoreInfoWindow.close();
                        }

                        primaryStoreInfoWindow = new google.maps.InfoWindow({
                            content: compiledTemplateStore({
                                "store": chance.city(),
                                "store-manager": chance.name(),
                                "phone": "+64 " + chance.phone()
                        ...
                        primaryStoreInfoWindow.open(map, this);
                        google.maps.event.addListener(primaryStoreInfoWindow, 'closeclick', function() {
                            if (buffer != undefined) {
                                buffer.setMap(null);
                            }
                        });
